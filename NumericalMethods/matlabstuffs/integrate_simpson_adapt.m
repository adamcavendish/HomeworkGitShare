function [ SRmat, quad, err ] = integrate_simpson_adapt( f, a, b, tol )
% Input
%	- f is the integrand input as a string `f`
%	- a and b are upper and lower limits of integration
%	- tol is the tolerance
% Output
%   - SRmat is the table of values
...
% SRvec = zeros(1, 6);
SRvec = simpson_improve(f, a, b, tol);
SRmat(1, 1:6) = SRvec;
...
while(state == iterating)
    ...
    for j = n : -1 : 1
        ...
        if(tol <= err)
            ...
            tol = SR0vec(6);
            tol2 = tol/2;
            SR1vec = simpson_improve(f, a, c, tol2);
            SR2vec = simpson_improve(f, c, b, tol2);
